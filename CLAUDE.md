...
Four loudspeaker design calculators served as a static multi-page site at
...
**THE TWO GINKGO TOOLS LEFT THIS REPO ON 2026-09-09** for
`ginkgo.kiiworkshop.com`, repo **`mcpjk/ginkgoMulticell`**, with their full
history: they had reached ~20,800 lines against these four tools' ~3,500, and
~99% of a five-minute build gate every unrelated change had to pay. What is
left here for them is two REDIRECT STUBS — `ginkgo-horn.html` and
`ginkgo-rim-lab.html`, static meta-refresh pages that keep existing links and
bookmarks working. They are real entries in `vite.config.js` and are covered
by `tests/calculators.spec.js`; do not delete them.
**`src/palette.js` is the ONE file the two repos share.** It was the only
import that crossed the boundary. It is duplicated, not published, so a
palette change has to be made in BOTH repos, and `npm run check:palette` runs
in both. That is the standing cost of the split.
Everything about the horn physics — the standing acoustic priority, the
findings, the task queue — went with the tools and now lives in that repo's
`CLAUDE.md`, `FINDINGS-ARCHIVE.md` and `NEXT-SESSION.md`. Do not re-derive it
here; go and read it there.

An earlier tool, CD Exit Cell Division, was deleted once the multicell tool
...
This file is short because the physics left with the Ginkgo tools. Keep it
that way: it is loaded in full at the start of every session here, and the
lesson from the Ginkgo side — where it reached 3650 lines, about 65k tokens
before anyone had typed anything — is that nothing enforces brevity except
deciding to.
