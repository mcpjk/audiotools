// Experimental snapshot of hgrid-model.js at c83f088; see rim-lab/README.md.
import { createRim } from "./rim-geometry.js";
// ═══════════════════════════════════════════════════════════════════════════
...
function interpSystem(tp, startDer = 2, endDer = 2) {
...
function interpSolve(sys, data, start = 0, end = 0) {
  const m = data.length - 1;
  const rhs = [data[0], start];
...
const interpSolve3 = (sys, pts, start = [0,0,0], end = [0,0,0]) => {
  const xs = interpSolve(sys, pts.map((p) => p[0]), start[0], end[0]);
  const ys = interpSolve(sys, pts.map((p) => p[1]), start[1], end[1]);
  const zs = interpSolve(sys, pts.map((p) => p[2]), start[2], end[2]);
...
// Rim sections need endpoint tangent AND curvature constraints. Natural cubic
// interpolation would silently erase the designed join curvature. A quintic
// interpolant uses four endpoint derivative constraints alongside the samples.
const rimSystems = new Map();
function rimSystem(m) {
  if (rimSystems.has(m)) return rimSystems.get(m);
  const knots = [...Array(6).fill(0), ...Array.from({length:m-1},(_,i)=>(i+1)/m), ...Array(6).fill(1)];
  const n=m+5, A=Array.from({length:n},()=>Array(n).fill(0));
  const row=(r,t,der)=>{const span=findSpan(knots,5,n,t), d=dersBasisFuns(span,t,5,der,knots)[der];
    for(let j=0;j<=5;j++) A[r][span-5+j]=d[j];};
  row(0,0,0); row(1,0,1); row(2,0,2);
  for(let q=1;q<m;q++) row(q+2,q/m,0);
  row(n-3,1,2); row(n-2,1,1); row(n-1,1,0);
  const sys={knots,lu:luFactor(A)}; rimSystems.set(m,sys); return sys;
}
function rimSolve(sys, pts, boundary) {
  const m=pts.length-1;
  const estimate=(end,order)=>{
    const weights=order===1?[-25/12,4,-3,4/3,-1/4]:[35/12,-26/3,19/2,-14/3,11/12];
    return [0,1,2].map(k=>weights.reduce((sum,w,i)=>sum+w*pts[end?m-i:i][k],0)*m**order*(end&&order===1?-1:1));
  };
  const [v0,a0,v1,a1]=boundary||[estimate(false,1),estimate(false,2),estimate(true,1),estimate(true,2)];
  const axes=[0,1,2].map(k=>{
    const rhs=[pts[0][k],v0[k],a0[k],...pts.slice(1,-1).map(p=>p[k]),a1[k],v1[k],pts[m][k]];
    const x=luSolve(sys.lu,rhs); x[0]=pts[0][k]; x[x.length-1]=pts[m][k]; return x;
  });
  return axes[0].map((_,i)=>axes.map(a=>a[i]));
}

// Evaluate a B-spline surface given its control net (net[i][j], i along u).
export function evalBsplineSurf(net, uKnots, vKnots, u, v) {
  const nu = net.length, nv = net[0].length, pu = uKnots.length - nu - 1, pv = vKnots.length - nv - 1;
  const su = findSpan(uKnots, pu, nu, u), sv = findSpan(vKnots, pv, nv, v);
  const Nu = dersBasisFuns(su, u, pu, 0, uKnots)[0];
  const Nv = dersBasisFuns(sv, v, pv, 0, vKnots)[0];
  const P = [0, 0, 0];
  for (let i = 0; i <= pu; i++)
    for (let j = 0; j <= pv; j++) {
      const w = Nu[i] * Nv[j], q = net[su - pu + i][sv - pv + j];
...
const greville = (knots, n) =>
  Array.from({ length: n }, (_, i) => { const p = knots.length - n - 1; return knots.slice(i + 1, i + p + 1).reduce((a,b)=>a+b,0)/p; });
...
export function ductBrep(sections, { capMouthPts = null, vParam = "chord" } = {}) {
  ...
  const rim = !!sections[0].rimBoundary;
  const sysU = rim ? rimSystem(mU) : interpSystemCached(mU);
  const startField = sections[0].rimVStart, endField = sections.at(-1).rimVEnd;
  const sysV = startField || endField ? interpSystem(vParams, startField ? 1 : 2, endField ? 1 : 2)
    : vParam === "uniform" ? interpSystemCached(mV) : interpSystem(vParams);
  if (!sysU.lu || !sysV.lu) return null;
  const nu = mU + (rim ? 5 : 3), nv = mV + 3;
  ...
  for (let s = 0; s < 4; s++)
    for (let q = 0; q < S; q++) {
      ...
      rowsU[s][q] = rim ? rimSolve(sysU, data, s === 0 ? sections[q].rimBoundary : null) : interpSolve3(sysU, data);
    }

  const derivativeNet = field => field ? Array.from({length:4},(_,side)=>
    rimSolve(sysU,Array.from({length:n+1},(_,k)=>field[(side*n+k)%N]),null)) : null;
  const startNet=derivativeNet(startField),endNet=derivativeNet(endField);
  ...
  const cornerCols = Array.from({ length: 4 }, (_, s) =>
    interpSolve3(sysV, Array.from({ length: S }, (_, q) => rowsU[s][q][0]), startNet?.[s][0], endNet?.[s][0]));
  const walls = Array.from({ length: 4 }, (_, s) => {
    const net = new Array(nu);
    net[0] = cornerCols[s];
    net[nu - 1] = cornerCols[(s + 1) % 4];
    for (let i = 1; i < nu - 1; i++)
      net[i] = interpSolve3(sysV, Array.from({ length: S }, (_, q) => rowsU[s][q][i]), startNet?.[s][i], endNet?.[s][i]);
    return net;
  });
...
function stepEmit({ name, desc, fileDesc, params, solidsSpec, folders = true, vParam = "chord" }) {
  ...
  const curveEnt = (ids, knots) => {
    ...
    return add(`B_SPLINE_CURVE_WITH_KNOTS('',${knots.length - ids.length - 1},(${ids.map((i) => "#" + i).join(",")}),.UNSPECIFIED.,.F.,.F.,${m},${v},.UNSPECIFIED.)`);
  };
  const surfEnt = (idNet, uKnots, vKnots) => {
    const ku = knotTxt(uKnots), kv = knotTxt(vKnots);
    const rows = idNet.map((col) => `(${col.map((i) => "#" + i).join(",")})`).join(",");
    return add(`B_SPLINE_SURFACE_WITH_KNOTS('',${uKnots.length - idNet.length - 1},${vKnots.length - idNet[0].length - 1},(${rows}),.UNSPECIFIED.,.F.,.F.,.F.,${ku.m},${kv.m},${ku.v},${kv.v},.UNSPECIFIED.)`);
  };
  ...
  for (const spec of solidsSpec) {
    ...
    checks.volumes.push({ brep: spec.group === "experimental rim" ? Math.abs(brepShellOrientation(brep, 24, 48).volume) : brepVolume(brep, sections), mesh: Math.abs(meshVolume(dm.verts, dm.tris)) });
...
export function buildShellSTEP(throat, map, {
  ...
  // Experimental rim: independent profiles and explicit corner patches.
  let flareOut = null;
  if (rim?.enabled && !only) {
    const fc = rimCollar(throat, map, { ...rim, t, wall, xSide, ySide });
    flareOut = fc?.report;
    if (!fc || !fc.report.ok) throw new Error(`Rim refused: ${fc?.report.why || 'missing aperture'}`);
    for (const solid of fc.solids) solidsSpec.push(solid);
  }
  ...
  const withFlare = nF ? `, then union the ${nF} 'experimental rim' piece(s) onto the rim (their root faces lie on the aperture; or print them separately)` : "";
  const recipe = (ends
    ? (only
  ...
  const flareStamp = rim?.enabled ? `rim-lab=${JSON.stringify(rim)}` : "rim-lab=off";
  const out = stepEmit({
...
// Exterior aperture boundary and incoming wall measurements.
const RIM_TOL = 1e-6;
...
export function rimExitField(throat, map, { t = 0 } = {}) {
  ...
  for (const cellRec of throat.cells) {
    ...
    for (let k = 0; k < last.length; k++) {
      ...
      const before = duct[Math.max(0, Q - 2)].pts[k];
      const dPrev = un3(s3(prev[k], before));
      const ds = 0.5 * (nrm3(s3(P, prev[k])) + nrm3(s3(prev[k], before)));
      for (const sd of sides) raw[sd].push({ P, a: pr.a, e: pr.e, d, dPrev, ds, cell: cellRec.label });
    }
  }
  const edges = {};
  ...
  for (const side of ["left", "right", "top", "bottom"]) {
    ...
    for (const arr of groups.values()) {
      ...
      const curvature = arr.reduce((sum, p) => {
        const now = Math.atan2(dot3(p.d, o), dot3(p.d, n));
        const old = Math.atan2(dot3(p.dPrev, o), dot3(p.dPrev, n));
        return sum + Math.atan2(Math.sin(now - old), Math.cos(now - old)) / Math.max(p.ds, 1e-9);
      }, 0) / arr.length;
      ...
      d = un3(d);
      st.push({ P: p0.P, a: p0.a, e: p0.e, o, n, tau, d, theta, curvature, kink, cells: arr.map((p) => p.cell) });
    ...
    if (st.length >= 2 && !st.some((p) => Math.abs(par(p)) < tolP)) {
      ...
      if (i > 0) {
        ...
        st.splice(i, 0, { P, a, e, o, n, tau: vertical ? tV : tH, d, theta, curvature: A.curvature + (B.curvature - A.curvature) * w, kink: 0, cells: [], interpolated: true });
      }
    }
    edges[side] = st;
  }
...
// Tangent-plane mismatch of the emitted air faces, excluding the retained
// corner pole. This is measured on the STEP surfaces, not just their rings.
export function rimJoinError(solids) {
  const cross=(a,b)=>[a[1]*b[2]-a[2]*b[1],a[2]*b[0]-a[0]*b[2],a[0]*b[1]-a[1]*b[0]];
  const normal=(b,u,v)=>{
    const h=1e-6, at=(u,v)=>evalBsplineSurf(b.walls[0],b.uKnots,b.vKnots,u,v);
    const U=s3(at(u+h,v),at(u-h,v));
    const V=s3(at(u,v===1?1:h),at(u,v===1?1-h:0));
    return un3(cross(U,V));
  };
  let worst=0;
  for(let i=0;i<solids.length;i+=3) {
    ...
    for(let j=0;j<2;j++) for(let k=1;k<32;k++) {
      ...
      worst=Math.max(worst,Math.acos(Math.min(1,Math.abs(dot3(a,c))))*180/Math.PI);
    }
  }
  return worst;
}
export function rimCollar(throat, map, options = {}) {
  const field = rimExitField(throat, map, options);
  if (!field) return null;
  const result=createRim(field, options);
  if(result.report.ok && result.report.on) {
    result.report.cornerJoinDeg=rimJoinError(result.solids);
    if(result.report.cornerJoinDeg>.1) {
      result.report.ok=false;
      result.report.why='Corner tangent-plane check failed; adjust the rim dimensions';
      result.solids=[];
    }
  }
  return result;
