...
// ═══════════════════════════════════════════════════════════════════════════
// GINKGO MULTICELL HORN — COCKPIT LAYOUT
// ═══════════════════════════════════════════════════════════════════════════
//
// A DELIBERATE DUPLICATE of GinkgoHorn.jsx carrying the same physics, state
// and readouts in a different physical layout, so the owner can compare the
// two side by side before choosing one. The classic page stays canonical
// until that choice is made; once it is, one of the two files is deleted —
// do not let them drift apart in ANY numeric behaviour meanwhile. Everything
// above the render (state, memos, solvers, exports, warnings) is copied
// verbatim from the classic page; only the JSX below `return (` differs.
//
// THE LAYOUT: two panes. The LEFT pane scrolls and carries the inputs as
// eight numbered stages in design chronology — driver, throat partition,
// coverage & mouth, expansion law, depth & path, path lengthening, a ghost
// slot for the planned coped joints, export — each stage housing its own
// diagram (throat plan in stage 2, mouth plan in stage 3, at the owner's
// call). The RIGHT pane is pinned: a header naming the horn, the warnings,
// a tabbed viewport (3-D ducts / horizontal section / path lengths / cell
// table), and a verdict strip that scrolls independently underneath —
// the acoustic behaviour and physical form of the horn being built, always
// on screen whatever the left side is editing.
//
...
// One numbered stage card in the left rail. The number is chronology, not
// decoration: each stage consumes what the previous one fixed.
...
export default function GinkgoCockpit() {
  // ── cockpit layout state ──
  // Which view the pinned pane shows, and whether the viewport is too narrow
  // for two panes at all — below the breakpoint the layout stacks back into
  // one scrolling column, which is roughly the classic page's behaviour.
  ...
  const views = [["ducts", "3-D ducts"], ["section", "horizontal section"], ["paths", "path lengths"], ["cells", "cells"]];
  const rightPane = (
    <div style={{
      ...
      <div style={{ margin: "0 12px", background: C.panel, border: `1px solid ${C.border}`, borderRadius: "0 4px 4px 4px", flex: "none", padding: 6 }}>
        ...
        {view === "paths" && (
          <div>
            <div style={{ display: "flex", gap: 8, alignItems: "baseline", flexWrap: "wrap", padding: "0 4px 4px" }}>
              <span style={{ fontFamily: C.mono, fontSize: 10, color: C.inkMuted }}>developed path per cell against the ΔL budget — the longest cell sets it</span>
            </div>
            <div style={{ opacity: stale ? 0.35 : 1 }}>{map ? pathSVG() : <div style={{ fontSize: 11, color: C.inkMuted, padding: 20 }}>No mapping to draw.</div>}</div>
          </div>
        )}
        {view === "cells" && (
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontFamily: C.mono, fontSize: 10, color: C.inkMuted }}>cockpit layout — same tool, rearranged</span>
        <a href="./ginkgo-horn.html" style={{ marginLeft: "auto", fontFamily: C.mono, fontSize: 10, color: C.series2 }}>
          compare with the classic page →
        </a>
      </div>
      <div style={narrow
        ? {}
