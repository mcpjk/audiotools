import { designKey } from "./rim-lab/state.js";
import { WorkerLane } from "./rim-lab/worker-client.js";
import React, { useState, useMemo, useEffect, useRef, useId } from "react";
import { C, SERIES } from "./palette.js";
import * as G from "./rim-lab/model.js";
...
//
// RIM LAB: independent circular, elliptical and curvature-matched profiles.
// Geometry is compared; no radiated field or diffraction improvement is predicted.
//
...
export default function GinkgoRimLab() {
  ...
  // Independent termination experiment; the cell aperture stays fixed.
  const [rimEnabled, setRimEnabled] = useState(true);
  const [rimFamily, setRimFamily] = useState("spline");
  const [rimWidth, setRimWidth] = useState(35);
  const [rimDepth, setRimDepth] = useState(35);
  const [rimEnd, setRimEnd] = useState("free");
  const [rimAngle, setRimAngle] = useState(150);
  ...
  const flareCfg = useMemo(() => ({ enabled: rimEnabled, family: rimFamily,
    width: rimWidth, depth: rimDepth, termination: rimEnd, endAngle: rimAngle, wall: shellWall,
  }), [rimEnabled, rimFamily, rimWidth, rimDepth, rimEnd, rimAngle, shellWall]);
  const flareOn = rimEnabled;
  const flareKey = designKey({ diagnosticKey, samples: checked?.samples || 0, flareCfg });
  const [flared, setFlared] = useState(null);
  const flare = flareOn && map && flared?.key === flareKey ? flared.value : null;
  useEffect(() => {
    if (stale || !flareOn || !checked) return;
    let active = true;
    const id = setTimeout(() => {
      lanes.flare.run("flare", flareKey, { ...diagnosticInput, options: { ...diagnosticInput.options, samples: checked.samples }, flare: flareCfg }).then(value => {
  ...
  const warnings = useMemo(() => {
    ...
    if (flare && flare.report.on && !flare.report.ok)
      w.push(`Experimental rim refused: ${flare.report.why}. Adjust the profile dimensions or shell wall.`);
    if (map && map.band !== "ok")
  ...
  // ── exports ────────────────────────────────────────────────────────────────
  const stem = `ginkgo_rim_lab_${fmt(shown.R * 2, 1)}mm_${shown.nc}x${shown.nr}_${throat.N}cells`;
  ...
  const runExport = async format => {
    ...
    try {
      const result = await lanes.diagnostics.run("export", diagnosticKey, {
        ...
        state: { tool: "ginkgo-rim-lab", layout: shown, achieved: solve.p, fabrication: { process, temperature },
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      ...
      // Draw the experimental material faintly over the air-passage preview.
  ...
  const leftPane = (
    <div style={{
      ...
      <Stage n={3} title="Coverage & mouth" why="the aperture is stated by what it must deliver — two arcs, each with its own angle and length">
        ...
        <div style={{ ...secTitle, marginTop: 12 }}>Experimental perimeter rim</div>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", margin: "8px 0" }}>
          <button style={btn(rimEnabled, C.accent)} onClick={() => setRimEnabled(v => !v)}>{rimEnabled ? "Rim on" : "Rim off"}</button>
          {["circle", "ellipse", "spline"].map(f => <button key={f} style={btn(rimFamily === f, C.accent)} onClick={() => setRimFamily(f)}>{f === "spline" ? "Curvature-matched spline" : f === "ellipse" ? "Ellipse" : "Circle"}</button>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          <NumInput label="Rim outward scale" disabled={!rimEnabled} value={rimWidth} onChange={setRimWidth} unit="mm" min={8} max={150} step={5} accent={C.accent} />
          <NumInput label="Rim forward scale" disabled={!rimEnabled || rimFamily === "circle"} value={rimDepth} onChange={setRimDepth} unit="mm" min={8} max={150} step={5} accent={C.series2} />
          <NumInput label="Rim final direction" disabled={!rimEnabled || rimEnd === "baffle"} value={rimAngle} onChange={setRimAngle} unit="°" min={100} max={175} step={5} accent={C.accentDim} />
        </div>
        <div style={{ display: "flex", gap: 6, margin: "8px 0" }}>
          <button style={btn(rimEnd === "free", C.accent)} onClick={() => setRimEnd("free")}>Freestanding rollback</button>
          <button style={btn(rimEnd === "baffle", C.accent)} onClick={() => setRimEnd("baffle")}>Baffle tangent</button>
          <a href="./ginkgo-horn.html" style={{ color: C.accent, fontSize: 11, alignSelf: "center" }}>Open original Ginkgo</a>
        </div>
        ...
        {flare?.report.ok && rimEnabled && <svg viewBox="-5 -10 160 100" role="img" aria-label="Rim profile comparison" style={{ width: "100%", height: 150, marginTop: 8 }}>
          {flare.report.profiles.map((p, i) => <g key={p.family}>
            <path d={p.path} fill="none" stroke={[C.series2, C.accentDim, C.accent][i]} strokeWidth={p.family === rimFamily ? 2 : 0.8} />
            <text x={i * 52} y={85} fontSize="5" fill={C.inkDim}>{p.family}</text>
          </g>)}
        </svg>}
        <div style={{ ...hintStyle, marginTop: 5 }}>
          Ellipse and spline share endpoints and end directions at the same scales. Circle uses the outward scale for both axes.
          Scales are profile axes in the incoming wall frame, not overall horn dimensions; compare the measured envelope above.
          The spline matches estimated incoming section curvature and tapers to zero curvature at its end.
          Baffle mode ends tangent to the local aperture extension at 90°; a curved mouth needs a matching curved baffle.
          Freestanding mode uses the final direction above, measured from the local aperture normal.
          Side and corner pieces meet at shared sections. Sharp aperture corners remain as pole patches, so this is not a globally smooth corner blend.
          The rim ships in the shell STEP kit; duct-only files contain air passages, not rim material.
          This comparison predicts geometry only. Cell loading/path metrics exclude the rim's acoustic interaction; no diffraction improvement is claimed.
        </div>
      </Stage>
      ...
      <Stage n={9} title="Export" why={`exports reuse the checked geometry; the preview stays at ${PREVIEW_STATIONS} stations`}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button style={expBtn} disabled={!canExport || (rimEnabled && !flare?.report.ok)} onClick={() => runExport("shell")}>STEP · horn shell</button>
        ...
        <div style={{ ...hintStyle, marginTop: 6 }}>
          ...
          {rimEnabled ? <> The experimental rim ships as separate side and corner pieces in its own folder.
            Root faces seat on the original aperture. Validate joining and the final exposed edge in CAD before fabrication.</> : null}
        </div>
      </Stage>
